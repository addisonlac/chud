...
//
//   1. 4h BIAS      — the higher timeframe sets the only direction we'll take.
//   2. 1h STRUCTURE — confirm the entry timeframe agrees (or just flipped to).
//   3. TRIGGER      — a liquidity sweep (stop hunt) is the primary entry; a
//                     fresh 1h break of structure is the secondary entry.
...
export function analyze(symbol: string, h1: Bar[], h4: Bar[], cfgIn: Partial<EngineConfig> = {}): Signal {
  ...
  if (h1.length < 30 || h4.length < 15 || !lastBar) {
    return blankSignal(symbol, price, generatedAt, "Insufficient history to analyse (need ≥30×1h and ≥15×4h bars).");
  }

  // ---- 1. Higher-timeframe (4h) bias ------------------------------------
  const htf = computeStructure(h4, cfg.swingStrength);
  const htfEma = last(ema(h4.map((b) => b.close), Math.min(cfg.emaPeriod, Math.floor(h4.length / 2))));
  const htfTrend: MarketTrend = htf.trend;
  const htfFreshChoch = htf.lastBreak?.event === "CHoCH" && htf.lastBreak.index >= h4.length - 3;

  // ---- 2. Entry-timeframe (1h) structure --------------------------------
  const ltf = computeStructure(h1, cfg.swingStrength);
  const pools = findLiquidityPools(ltf.swings, { equalTolerancePct: cfg.equalTolerancePct });
  const sweeps = detectSweeps(h1, ltf.swings, cfg.swingStrength, { equalTolerancePct: cfg.equalTolerancePct });
  const fvgs = findFvgs(h1);

  const atrSeries = atr(h1, cfg.atrPeriod);
  const atrNow = last(atrSeries) ?? price * 0.01;
  const rsiNow = last(rsi(closes1h, cfg.rsiPeriod));
  const emaNow = last(ema(closes1h, cfg.emaPeriod));
  ...
  if (!dir) {
    const steps: ReasoningStep[] = [
      ...
      { label: "1h trigger", detail: "No liquidity sweep or break of structure in the last few bars — nothing to act on.", verdict: "neutral", weight: 0 },
    ];
    return { ...blankSignal(symbol, price, generatedAt, "No entry trigger — waiting for a sweep or 1h break of structure."), htfTrend, ltfTrend: ltf.trend, reasoning: steps, evidence: { sweep: null, orderBlock: null, fvg: null, structureBreak: ltf.lastBreak, liquidityPools: pools.slice(0, 6) } };
  }
  ...
  if (biasCounter && !htfFreshChoch) {
    ...
    reasoning.push({ label: "Counter-trend", detail: `Setup fights the 4h ${htfTrend} bias with no 4h change-of-character yet — confidence penalised.`, verdict: "neutral", weight: 0 });
  }
  if (htfEma !== null) {
    const emaOk = (isLong && price >= htfEma) || (!isLong && price <= htfEma);
    add({ label: "4h EMA", detail: `Price ${price >= htfEma ? "above" : "below"} the 4h trend EMA (${htfEma.toFixed(2)}) — ${emaOk ? "supports" : "argues against"} the ${isLong ? "long" : "short"}.`, verdict: emaOk ? (isLong ? "bullish" : "bearish") : "neutral", weight: 0.08 }, emaOk);
  }
  ...
  if (triggerKind === "sweep" && recentSweep) {
    ...
    add({ label: "Break of structure", detail: `Fresh 1h ${recentBreak.event} ${recentBreak.direction} — price closed ${isLong ? "above" : "below"} ${recentBreak.brokenSwingPrice.toFixed(2)}, confirming ${isLong ? "upside" : "downside"} intent.`, verdict: isLong ? "bullish" : "bearish", weight: 0.18 }, true);
  }

  // 4c. 1h structure agreement
  const ltfAgrees = (isLong && ltf.trend === "bullish") || (!isLong && ltf.trend === "bearish");
  add({ label: "1h structure", detail: `Entry timeframe is ${ltf.trend}${ltfAgrees ? " — aligned with the trade." : "."}`, verdict: ltf.trend === "ranging" ? "neutral" : ltf.trend === (isLong ? "bullish" : "bearish") ? (isLong ? "bullish" : "bearish") : "neutral", weight: 0.12 }, ltfAgrees);

  // 4d. Order block reaction
  const obBreak = ltf.lastBreak ?? recentBreak;
  const ob = obBreak ? orderBlockForBreak(h1, obBreak) : null;
  ...
  // 4e. FVG confluence
  const fvg = latestUnfilledFvg(h1, fvgs, isLong ? "bullish" : "bearish");
  if (fvg) {
  ...
  // 4f. Momentum (RSI + EMA on 1h)
  if (rsiNow !== null) {
    const rsiOk = isLong ? rsiNow > 45 && rsiNow < 72 : rsiNow < 55 && rsiNow > 28;
    const extreme = isLong ? rsiNow >= 72 : rsiNow <= 28;
    add({ label: "RSI", detail: `1h RSI ${rsiNow.toFixed(0)} — ${extreme ? (isLong ? "overbought, chase risk" : "oversold, chase risk") : rsiOk ? "healthy momentum for the entry" : "momentum not yet confirming"}.`, verdict: rsiOk ? (isLong ? "bullish" : "bearish") : "neutral", weight: 0.1 }, rsiOk);
  }
  if (emaNow !== null) {
    const emaOk = (isLong && price >= emaNow) || (!isLong && price <= emaNow);
    add({ label: "1h EMA", detail: `Price ${price >= emaNow ? "above" : "below"} the 1h EMA (${emaNow.toFixed(2)}).`, verdict: emaOk ? (isLong ? "bullish" : "bearish") : "neutral", weight: 0.05 }, emaOk);
  }

  score = Math.max(0, Math.min(1, score - counterTrendPenalty));

  // ---- 5. Risk model: entry / stop / targets ----------------------------
  const swingLow = ltf.lastSwingLow?.price ?? Math.min(...h1.slice(-10).map((b) => b.low));
  const swingHigh = ltf.lastSwingHigh?.price ?? Math.max(...h1.slice(-10).map((b) => b.high));
  ...
  // ---- 6. Decision ------------------------------------------------------
  // Top-down discipline: only take a trade WITH a clear 4h trend behind it, or
  // on a just-formed 4h change-of-character (a reversal). No trading a ranging
  // or opposing higher timeframe — that's where these setups go to die.
  let action: Action = "WAIT";
  const rrOk = riskReward !== null && riskReward >= cfg.minRiskReward;
  const biasOk = biasAligned || htfFreshChoch;
  const gatesOk = score >= cfg.minConfidence && rrOk && validEntry && biasOk;
...
function biasStep(trend: MarketTrend, freshChoch: boolean): ReasoningStep {
  const detail = freshChoch
    ? `4h just printed a change-of-character to ${trend} — a potential regime shift.`
    : `4h market structure is ${trend}.`;
  return { label: "4h bias", detail, verdict: trend === "ranging" ? "neutral" : trend, weight: 0.25 };
}
